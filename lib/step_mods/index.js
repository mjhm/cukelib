...
const stepMods = {
  initialize() {
    if (this._cukeapiEmitter) return;
    this._cukeapiEmitter = new EventEmitter();

    const origThen = this.Then;
    this.Then = function (...args) {
      origThen.apply(this, args);
      this._cukeapiEmitter.emit('thenEvent', this, origThen, ...args);
    };

    const origGiven = this.Given;
    this.Given = function (...args) {
      origGiven.apply(this, args);
      this._cukeapiEmitter.emit('givenEvent', this, origGiven, ...args);
    };

    const origWhen = this.When;
    this.When = function (...args) {
      origWhen.apply(this, args);
      this._cukeapiEmitter.emit('whenEvent', this, origWhen, ...args);
    };
  },

  withThenNotSteps(stepDefinitionFn) {
    stepMods.initialize.call(this);
    this._cukeapiEmitter.on('thenEvent', stepMods.createNotStep);
    stepDefinitionFn.call(this);
    this._cukeapiEmitter.removeListener('thenEvent', stepMods.createNotStep);
  },
    ...
    const genericWrap = function (...args) {
      ...
      try {
        ...
        if (errorClass && !(error instanceof errorClass)) {
          const shortErrClass = errorClass.name || errorClass;
          assertFail(
            `Expected thrown error to be type ${shortErrClass} but was ${error.stack || error}`
          );
        }
        return error;
      }
      ...
      return result.catch((error) => {
        if (errorClass && !(error instanceof errorClass)) {
          const shortErrClass = errorClass.name || errorClass;
          assertFail(
            `Expected rejected error to be type ${shortErrClass} but was ${error.stack || error}`
          );
        }
        throwMe = false;
        return error;
        ...
        if (throwMe) {
          assertFail(
            msg || `Expected promise from ${fn} to be rejected but was resolved with ${resolution}`
          );
        }
        return resolution;
      });
