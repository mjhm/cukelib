...
const stepMods = {
  initialize() {
    if (this._cucapiEmitter) return;
    this._cucapiEmitter = new EventEmitter();

    const origThen = this.Then;
    this.Then = function (...args) {
      origThen.apply(this, args);
      this._cucapiEmitter.emit('thenEvent', this, origThen, ...args);
    };

    const origGiven = this.Given;
    this.Given = function (...args) {
      origGiven.apply(this, args);
      this._cucapiEmitter.emit('givenEvent', this, origGiven, ...args);
    };

    const origWhen = this.When;
    this.When = function (...args) {
      origWhen.apply(this, args);
      this._cucapiEmitter.emit('whenEvent', this, origWhen, ...args);
    };
  },

  withThenNotSteps(stepDefinitionFn) {
    stepMods.initialize.call(this);
    this._cucapiEmitter.on('thenEvent', stepMods.createNotStep);
    stepDefinitionFn.call(this);
    this._cucapiEmitter.removeListener('thenEvent', stepMods.createNotStep);
  },
