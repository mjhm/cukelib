...
const mktags = cucumber.defineSupportCode ? (arr) => arr.join(' or ') : (arr) => arr;


const { get, set, unset, universeGet, getCukeContext, initializeWith } =
  universe.namespaceFactory('_internal_universe_test');
const other = universe.namespaceFactory('_other_test_namespace');

const internalUniverseTest = function () {
  ...
  const setResult = set('myUniverseKey1', 'myUniverseValue1');
  expect(setResult, 'set result should be original value').to.equal('myUniverseValue1');
  ...
  this.Before({ tags: mktags(['@InternalUniverseTest']) }, () => {
...
if (cucumber.defineSupportCode) {
  cucumber.defineSupportCode((context) => {
    internalUniverseTest.call(context);
  });
}

module.exports = internalUniverseTest;
