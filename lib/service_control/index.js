...
const serviceControl = {
  ...
  launchService(name, start) {
    ...
    .then(start.bind(null, name))
    .then((service) => {
      if (!service.name) throw new Error('service is missing a name');
      if (!service.stop || !_.isFunction(service.stop)) {
        throw new Error(`service '${service.name}' is missing a stop function`);
      }
      set(`services.${name}`, service);
      service.context = getContext(); // eslint-disable-line no-param-reassign
      return service;
    });
  },
  ...
  embedServer(embedArgs = {}) {
    ...
    return serviceControl.stopService(host)
    .then(() => {
