...
const { get, set, unset, universeGet, getContext } =
...
module.exports = function () {
  ...
  expect(getContext()).to.equal('universe');
  ...
  this.registerHandler('BeforeFeature', () => expect(getContext()).to.equal('feature'));
  ...
  this.Before({ tags: ['@InternalUniverseTest'] }, () => {
    ...
    // the universe.
    expect(getContext()).to.equal('scenario');
  ...
  this.Then(/internal test of universe in the context of a world, scenario 1/, function () {
    ...
    expect(get('myShadowKeyToUnset1'), 'unsets in world context').to.be.undefined;
    expect(universeGet('myShadowKeyToUnset1'), 'still set in universe context')
