...
const stepMods = {
  ...
  withThenNotSteps(stepDefinitionFn: Function) {
    if (!this.Given) {
      throw new Error(
        'withThenNotSteps must be called in the cucumber context ' +
        'that contains step definers such as this.Given'
      );
    }
  ...
  withThrowSteps(stepDefinitionFn: Function) {
    if (!this.Given) {
      throw new Error(
        'withThrowSteps must be called in the cucumber context ' +
        'that contains step definers such as this.Given'
      );
    }
    stepMods.initialize.call(this);
