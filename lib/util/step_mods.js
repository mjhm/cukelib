...
// const assertFail = assert.fail.bind(assert, null, null);
    ...
    const genericWrap = function (...args) {
      ...
      try {
        ...
        if (errorClass && !(error instanceof errorClass)) {
          assertFail(`Expected thrown error to be type ${errorClass} but was ${error}`);
        }
        return error;
      }
      ...
      return result.catch((error) => {
        if (errorClass && !(error instanceof errorClass)) {
          assertFail(`Expected rejected error to be type ${errorClass} but was ${error}`);
        }
        throwMe = false;
        return error;
      }).then((resolution) => {
