...
module.exports = function () {
  ...
  this.registerHandler('BeforeFeatures', function () {
    // The unverse functions are still in the cukeContext of the "universe" inside "...Features" hooks.
    expect(getCukeContext()).to.equal('universe');
  ...
  this.Before({ tags: ['@InternalUniverseTest'] }, () => {
    // The get/set functions are now in the cukeContext of the seenario's World which was copied from
    // the universe.
  ...
  this.Then(/internal test of universe in the cukeContext of a world, scenario 1/, function () {
    expect(getCukeContext()).to.equal('scenario');
    ...
    expect(get('myShadowKey1'), 'value altered from universe cukeContext').to.equal('alteredValue1');
    expect(get('myShadowKey2'), 'value altered from world cukeContext').to.equal('alteredValue2');
    expect(get(), 'copy of universe from world "this"').to.equal(this._internal_universe_test);
    expect(universeGet(), 'access to universe from the world cukeContext').to.equal(universeTestNS);
    ...
    expect(get('myShadowKeyToUnset1'), 'unsets in world cukeContext').to.be.undefined;
    expect(universeGet('myShadowKeyToUnset1'), 'still set in universe cukeContext')
      .to.equal('myShadowValueToUnset1');
  });

  this.Then(/internal test of universe in the cukeContext of a world, scenario 2/, () => {
    expect(get('myUniverseKey1'), 'simple get from a universe value').to.equal('myUniverseValue1');
    expect(get('myWorldKey1'), 'simple get from a world value').to.equal('myWorldValue1');
  ...
  this.After(() => {
    // Set this in the world cukeContext to show that it persists in the AfterFeatures.
    expect(getCukeContext()).to.equal('scenario');
